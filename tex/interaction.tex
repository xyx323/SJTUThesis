...
\begin{figure}[!htp]
  ...
  \label{fig:NLI}
\end{figure}
%DONE: 上图太大，其字体应小于或等于正文字体，另外，图要加个外框（暂时没找到加框的方法）。
...
因此系统将根据算法和规则，修正错误关系，补全缺失节点和缺失关系，得到较为准确的解析树，称为查询树。详见\ref{nli:jxsyhcg}节。
    ...
    %DONE：查询结果返回给用户吗？在解决方案的图中没有这个信息
\end{enumerate}
...
\section{实验与分析}
%DONE: 在此要写出实验目的，有必要时可以列出RQ（研究问题），后续在实验分析时对各个RQ进行讨论与回答
本节是基于映射的INL2SQL解决方案的实验部分，将介绍实验所使用的评价指标和数据集并对实验结果进行分析，验证了方法的有效性和准确性。
\subsection{评价指标}
%done: 在此要写出SQL语句生成的准确性度量指标
...
\begin{table}[!hpb]
  ...
  \end{tabular}
\end{table}
%DONE：上表中增加一列“数据集”，指明是Classicmodels 数据集或MAS 数据集。
